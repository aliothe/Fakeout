...
void Level::generate_rectangular()
{
  ...
  for (int row = 0; row < rows_; ++row)
  {
    for (int col = 0; col < cols_; ++col)
    {
      ...
      float pos_x = start_x + (static_cast<float>(col) * config_.brick_width);
      float pos_y = config_.top_offset + (static_cast<float>(row) * config_.brick_height);

      auto props = resolve_brick_type(brick_type);
      create_brick(pos_x, pos_y, props.texture, props.color, props.hit_points);
    }
  }
}
...
void Level::generate_circular()
{
  ...
  constexpr float MIN_CIRCLE_RADIUS = 120.0F;
  radius = std::max(radius, MIN_CIRCLE_RADIUS);
  ...
  for (int ring = 0; ring < rows_; ++ring)
  {
    ...
    int max_bricks = cols_ + (ring * CIRCLE_COLS_GROWTH_PER_RING);
    ...
    for (int i = 0; i < bricks_in_ring; ++i)
    {
      ...
      angle -= std::numbers::pi_v<float> / ANGLE_OFFSET_FACTOR;
      float pos_x = center_x + (current_radius * std::cos(angle));
      float pos_y = center_y + (current_radius * std::sin(angle));
      ...
      if (pos_x < config_.brick_width / CENTER_DIVISOR ||
          pos_x > WINDOW_WIDTH_F - config_.brick_width / CENTER_DIVISOR || pos_y < SCORE_AREA_BOTTOM ||
          pos_y > PADDLE_AREA_TOP)
      {
        continue;
      }
...
auto Level::resolve_brick_type(BrickType type) const -> BrickProperties
{
  switch (type)
  {
  case BrickType::Red:
    return {.hit_points = 3, .texture = textures_.brick_red, .color = ecs::BrickColor::RED};
  case BrickType::Yellow:
    return {.hit_points = 2, .texture = textures_.brick_yellow, .color = ecs::BrickColor::YELLOW};
  case BrickType::Blue:
  default:
    return {.hit_points = 1, .texture = textures_.brick_blue, .color = ecs::BrickColor::BLUE};
  }
}

void Level::create_brick(float pos_x, float pos_y, SDL_Texture *texture, ecs::BrickColor color,
                         int hit_points)
{
  using namespace ecs;

  auto brick = registry_.create_entity();
  if (brick == INVALID_ENTITY)
  {
    return;
  }

  registry_.add_component<TransformComponent>(
      brick, TransformComponent{.position = {.x = pos_x, .y = pos_y}, .width = config_.brick_width, .height = config_.brick_height});

  registry_.add_component<SpriteComponent>(brick, SpriteComponent{.texture = texture, .tint = DEFAULT_TINT_VEC4});
  registry_.add_component<BrickComponent>(brick, BrickComponent{.hit_points = hit_points, .color = color});
}
